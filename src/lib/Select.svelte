<script>
	...
	const chevron = (opts) => `<svg class="${opts?.className}" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" viewBox="0 0 11.75 7.7" width="18" style="z-index:1"><path fill="currentColor" d="m1.37.15 4.5 5.1 4.5-5.1a.37.37 0 0 1 .6 0l.7.7a.45.45 0 0 1 0 .5l-5.5 6.2a.37.37 0 0 1-.6 0l-5.5-6.1a.64.64 0 0 1 0-.6l.7-.7a.64.64 0 0 1 .6 0Z"></path></svg>`;
	...
	function suggestionTemplate (result) {
		return result && (groupKey ? `${result[labelKey]} <span class="muted-text">${result[groupKey]}</span>` : result[labelKey]);
	}
...
<svelte:head>
	<link rel="stylesheet" href="https://alphagov.github.io/accessible-autocomplete/dist/accessible-autocomplete.min.css"/>
</svelte:head>

{#if label}<label for="{id}" style:display={hideLabel ? 'none' : null}>{label}</label>{/if}
<div class="ons-autocomplete-wrapper">
	<div id="{id}-container" class="ons-autocomplete" class:hide-menu={hideMenu} use:initAutocomplete></div>
