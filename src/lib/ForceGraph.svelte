<script>
  ...
  const _domain = Array.from(new Set(Object.values(nodes).map(n => n.continent))).sort((a, b) => a.localeCompare(b));
  const _range = Object.keys(_domain).map(k => +k * 2);
  const _scale = scaleOrdinal(_domain, _range);

  $: _proj = geoCylindricalEqualArea()
    .fitExtent(
  ...
  async function getGeoJSON() {
    const json = await (await fetch("https://raw.githubusercontent.com/mtraynham/natural-earth-topo/refs/heads/master/topojson/ne_110m_admin_0_countries.json")).json();
  ...
  function init() {
    _links = links.filter(l => selected.includes(l.source) || selected.includes(l.target)).map(l => ({...l}));
    _ids = new Set(_links.map(l => [l.source, l.target]).flat());
		_nodes = [..._ids].map(id => ({...nodes[id]}));
		
	  // Create a simulation with several forces.
	  simulation = forceSimulation(_nodes)
	      .force("link", forceLink(_links).id(d => d.id))
    ...
    if (simulation && selected.length !== _selected.length) {
      const oldLinks = _links.filter(l => selected.includes(l.source.id) || selected.includes(l.target.id));
      const oldLinkIds = new Set(oldLinks.map(l => l.id));
      const newLinks = links
        .filter(l => !oldLinkIds.has(l.id) && (selected.includes(l.source) || selected.includes(l.target)));
      _ids = new Set([
      ...
      const newNodes = [..._ids].filter(id => !oldIds.has(id)).map((id, i) => ({
        ...nodes[id],
        index: (oldNodes[oldNodes.length - 1]?.index || 0) + i
      }));
      _nodes = [...oldNodes, ...newNodes];
      _links = [...oldLinks, ...newLinks.map((l, i) => ({
        id: l.id,
...
<svg viewBox="{-width / 2} {-height / 2} {width} {height}" bind:this={svg} bind:clientWidth={width} bind:clientHeight={height}>
  <g transform={transform?.toString()}>
    ...
    <g>
      {#each _links as l (l.id)}
        <Line
          {mode}
          xy1={mode === "map" ? _proj([l.source.lng, l.source.lat]) : [l.source.x, l.source.y]}
          xy2={mode === "map" ? _proj([l.target.lng, l.target.lat]) : [l.target.x, l.target.y]}/>
      {/each}
    </g>
    <g>
      {#each _nodes as n (n.id)}
        <Circle
          ...
          fill={schemePaired[_scale(n.continent) + (selected.includes(n.id) ? 1 : 0)]}
          title={n.name}
...
<ul id="legend">
  {#each _domain as item, i}
    <li>
      <span class="bullet bullet-left" style:background={schemePaired[_range[i] + 1]}></span><span class="bullet bullet-right" style:background={schemePaired[_range[i]]}></span>
      {item}
    </li>
  {/each}
</ul>
